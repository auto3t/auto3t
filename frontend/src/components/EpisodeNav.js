...
const EpisodeNav = ({ currentEpisodeId }) => {
  ...
  return (
    <div className='episode-nav-container'>
      <div className="episode-nav">
        {previousEpisode ? (
          <Link className='first-nav' title='previous episode' to={`/media/episode/${previousEpisode.id}`}>
            <span>&#65308; </span>
            <span>S{String(previousEpisode.season.number).padStart(2, '0')}</span>
        ...
        {nextEpisode ? (
          <Link className='next-nav' title='next episode' to={`/media/episode/${nextEpisode.id}`}>
