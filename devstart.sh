...
if [ $? != 0 ]; then
    ...
    tmux send-keys -t $SESSION_NAME:"dev" "docker compose pull auto3t-redis auto3t-prowlarr flaresolverr auto3t-jellyfin auto3t-transmission && docker compose up -d auto3t-redis auto3t-prowlarr flaresolverr auto3t-jellyfin auto3t-transmission && docker compose logs -f" C-m
    wait_for_redis
    ...
    tmux send-keys -t $SESSION_NAME:"dev.1" "source .venv/bin/activate && cd backend && python manage.py rqworker --with-scheduler default show movie people thumbnails" C-m
